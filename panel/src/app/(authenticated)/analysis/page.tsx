...
export default function AnalysisPage() {
  ...
  const [activeTab, setActiveTab] = useState<"investigate" | "threat" | "osint">("investigate");
  const [osintTool, setOsintTool] = useState("whois");
  const [osintValue, setOsintValue] = useState("");
  const [threatType, setThreatType] = useState("ip");
  const [threatValue, setThreatValue] = useState("");
...
• "Check if 45.33.32.156 is malicious"`,
  ...
  const handleInvestigate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const recs = data.recommendations?.length
        ...
        : "";
      const fallbackNote = data.status === "completed_with_fallback"
        ? "\n\n⚠️ AI provider unavailable. Displaying deterministic fallback assessment."
        : "";
      ...
      setMessages((prev) => [...prev, {
        role: "assistant",
        content: data.analysis + fallbackNote + recs + toolInfo,
        timestamp: new Date(),
      ...
      setMessages((prev) => [...prev, {
        role: "assistant",
        content: "⚠️ " + (err?.message || "Unknown error") + "\n\nCheck API connectivity and AI provider configuration.",
        timestamp: new Date(),
      }]);
    } finally {
      setIsLoading(false);
  ...
  const handleThreatLookup = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!threatValue.trim() || isLoading) return;

    const val = threatValue.trim();
    setIsLoading(true);
    setMessages((prev) => [...prev, {
      role: "user",
      content: `🛡️ Threat Lookup (${threatType}): ${val}`,
      timestamp: new Date(),
    }]);
    setThreatValue("");

    try {
      const result = await api.threatLookup(threatType, val);
      const detailsPreview = Object.keys(result.details || {}).length
        ? "\n\nDetails:\n" + JSON.stringify(result.details, null, 2).slice(0, 1400)
        : "";
      ...
      setMessages((prev) => [...prev, {
        role: "assistant",
        content: `Threat level for ${threatType}:${val} → ${result.threat_level}${detailsPreview}${recommendations}`,
        timestamp: new Date(),
      }]);
    } catch (err: any) {
      setMessages((prev) => [...prev, {
        role: "assistant",
        content: "⚠️ Threat lookup failed: " + (err?.message || "Unknown error"),
        timestamp: new Date(),
      }]);
    } finally {
      setIsLoading(false);
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={"px-3 py-1.5 rounded-lg text-xs font-medium transition-all " +
              (activeTab === "threat" ? "bg-sentinel-600 text-white" : "bg-cyber-card text-cyber-muted border border-cyber-border")}
          >
            <Shield className="w-3 h-3 inline mr-1" />Threat Assessment
          </button>
          <button
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-cyber-muted">
                  {activeTab === "investigate"
                    ? "Analyzing with LLM (may use OSINT tools)..."
                    : activeTab === "threat"
                      ? "Running threat assessment lookup..."
      ...
      {activeTab === "investigate" ? (
        <form onSubmit={handleInvestigate} className="space-y-2">
          <div className="flex justify-start">
            <button
              type="button"
              onClick={() => setQuery("Perform a full cybersecurity threat assessment on my system. Include active alerts, suspicious telemetry, likely attack techniques, and prioritized remediation.")}
              className="text-[11px] px-2.5 py-1.5 bg-cyber-hover border border-cyber-border rounded text-cyber-muted hover:text-white"
            >
              Use system assessment prompt
            </button>
          </div>
        ...
        <form onSubmit={handleThreatLookup} className="space-y-2">
          <div className="flex gap-2">
            {["ip", "domain", "url", "hash_sha256", "hash_md5", "cve", "email"].map((t) => (
              <button
                key={t}
                type="button"
          ...
          <div className="flex gap-3">
            <input
              type="text"
              value={threatValue}
              onChange={(e) => setThreatValue(e.target.value)}
              placeholder={`Enter ${threatType} value for threat assessment`}
            ...
            <button
              type="submit"
              disabled={!threatValue.trim() || isLoading}
